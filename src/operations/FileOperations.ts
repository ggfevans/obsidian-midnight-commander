...
export class FileOperations {
	...
	private async executeCopyOperation(
		...
		const progressNotification =
			files.length > 3
				? NotificationManager.progress(`Copying files...`)
				: null;
	...
	private async executeMoveOperation(
		...
		console.log(
			`[FileOperations] Starting batch move of ${files.length} files to ${targetFolder.path}`
		);
		...
		try {
			...
			await runInBatch(
				this.app,
				files.map(file => (batch: BatchOperations) => {
	...
	async createNewFolder(
		...
		const folderName =
			name || (await this.promptForName('New folder name:', 'New Folder'));
	...
	async copyFileInPlace(file: TAbstractFile): Promise<void> {
		...
		try {
			...
			if (file instanceof TFile) {
				...
				const targetFolderObj = this.app.vault.getAbstractFileByPath(
					targetPath
				) as TFolder;
	...
	private generateUniqueTargetPath(
		file: TAbstractFile,
		targetFolder: TFolder
	): string {
